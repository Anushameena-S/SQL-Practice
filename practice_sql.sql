...
-- DDL (Data Definition Language) - Any operation, such as creating, modifying, or deleting database objects
-- DDL Commands - Create, Alter (Add, Change, Modify, Drop ),  Drop, Truncate
...
-- Date and Time: DATE, TIME, DATETIME (YYYY-MM-DD HH:MI:SS) 
...
-- to delete the contents inside a table, use Truncate.
TRUNCATE TABLE customers;
select * from customers;

-- Update the data for one row
UPDATE customers SET first_name = 'John', last_name= 'Kent' WHERE CustomerId=1;
...
-- TCL Commands
-- By default, the auto commit is on, so we are switching it off in below query
set autocommit=0;
...
-- Specify column name, Fetch the first name and country from customers
...
-- '%' represents any number of characters , but '_' represents only one character
...
-- offset ignores the first given number of records or rows from the table and 
-- limit up to the number given for limit and fetch.
select * from employees
limit 2 offset 5;
...
-- Set Operations - used to fetch info from more than 1 table
...
-- union all lists duplicates rows and union excludes duplicate rows
